...
interface DataContextType {
  ...
  alterarStatusPaciente: (id: string, novoStatus: StatusPaciente, usuarioNome: string) => void
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const getStats = useCallback((): DashboardStats => {
    ...
    const cotacoesVencidas = cotacoes.filter(
      c => c.dataValidade < hoje && c.status !== StatusCotacao.EXPIRADA
    ...
    return {
      ...
      pacientesSuspensos: pacientes.filter(p => p.status === StatusPaciente.SUSPENSO).length,
      pacientesEncerrados: pacientes.filter(p => p.status === StatusPaciente.ENCERRADO).length
    }
  ...
  const updatePaciente = useCallback((id: string, dados: Partial<Paciente>) => {
    setPacientes(prev =>
      prev.map(p => (p.id === id ? { ...p, ...dados, atualizadoEm: new Date().toISOString() } : p))
    )
  }, [])

  const alterarStatusPaciente = useCallback((id: string, novoStatus: StatusPaciente, usuarioNome: string) => {
  ...
  // Fornecedores
  const getFornecedorById = useCallback((id: string) => {
  ...
  // Cotações
  const getCotacaoById = useCallback((id: string) => {
    return cotacoes.find(c => c.id === id)
  }, [cotacoes])

  const getCotacoesByPaciente = useCallback((pacienteId: string) => {
  ...
  // Atendimentos
  const getAtendimentoById = useCallback((id: string) => {
    return atendimentos.find(a => a.id === id)
  }, [atendimentos])

  const getAtendimentosByPaciente = useCallback((pacienteId: string) => {
  ...
  // Histórico
  const getHistoricoByPaciente = useCallback((pacienteId: string) => {
    return historico
  ...
  // Documentos
  const getDocumentosByPaciente = useCallback((pacienteId: string) => {
  ...
  const removeDocumento = useCallback((id: string, usuarioNome: string, pacienteId: string) => {
